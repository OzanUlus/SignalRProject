...
using SignalRWeb.Dtos.ContactDtos;
using System.Text;
...
namespace SignalRWeb.Controllers
{
    public class ContactController : Controller
    {
        ...
        {
            var client = _httpClientFactory.CreateClient();
            var responseMessage = await client.DeleteAsync($"https://localhost:7233/api/Contact/{id}");
            if (responseMessage.IsSuccessStatusCode)
            {
                return RedirectToAction("Index");
            }
        ...
        public async Task<IActionResult> UpdateContact(UpdateContactDto updateContactDto )
        {
            ...
            var responseMassage = await client.PutAsync("https://localhost:7233/api/Contact", stringContent);
